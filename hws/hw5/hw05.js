...
function render() {
    ...
    const angle = cameraCircleSpeed * elapsedTime;
    const halfAngle = angle % 180;

    // Viewing transformation matrix
    let camX = cameraCircleRadius * Math.sin(glMatrix.toRadian(angle));
    let camZ = cameraCircleRadius * Math.cos(glMatrix.toRadian(angle));
    //let camY = cameraCircleRadius * Math.sin(glMatrix.toRadian(halfAngle));
    let camY = Math.cos(glMatrix.toRadian(60)); // => camY = 0
